 ...
 * рендер (аудит вьюх): cart_items на /cart, profiles/addresses на /checkout,
 * addresses на /account/addresses. Контроллеры теперь переиспользуют уже
 * прогретые CartManager/коллекции вместо повторного чтения тех же строк.
 */
class DuplicateQueryAuditTest extends TestCase
{
    ...
    public function test_cart_page_queries_cart_items_once(): void
    {
    ...
    public function test_checkout_page_queries_cart_items_profiles_and_addresses_once_each(): void
    {
        $user = User::factory()->create();
        ...
        CartItem::factory()->create([
            'cart_id' => $cart->id,
            'product_id' => Product::factory()->create(['in_stock' => true, 'stock_quantity' => 5]),
        ]);
