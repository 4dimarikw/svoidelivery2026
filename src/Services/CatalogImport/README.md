...
## Обнуление пропавших товаров

1С не гарантирует, что снятый с продажи товар вообще попадёт в выгрузку — он
просто исчезает из CSV. `PersistProductStage` работает построчно и ничего не
знает про строки, которых в файле нет, поэтому без отдельного шага такой
товар навсегда остаётся с последним известным остатком и продолжает
продаваться на сайте.

- **Сбор кодов.** `SeenCodeCollector` пишет `КодТовара` каждой строки в
  таблицу `catalog_import_seen_codes` **до** пайплайна стейджей — попадание
  в файл, а не факт успешного импорта. Отбракованная строка (например,
  `NormalizeRowStage` отсеял её по цене или категории) всё равно считается
  "виденной": иначе ошибка в реестре категорий или в правилах фильтрации
  обнулила бы часть каталога, которую 1С по-прежнему честно выгружает.
- **Обнуление.** После успешного импорта `CatalogImportCommand` ставит в
  очередь `Infrastructure\Jobs\ZeroOutStaleProductsJob`, который вызывает
  `Domain\Catalog\Actions\ZeroOutStaleProductsAction` — товарам без строки в
  `catalog_import_seen_codes` выставляется `stock_quantity = 0`,
  `in_stock = false`. Через `Eloquent::update()`, не массовый query-builder
  update — `in_stock` входит в `Product::SEO_WATCHED_ATTRIBUTES`, и только
  так поднимется хук пересинхронизации SEO.
- **Когда не выполняется:**
  - `--dry-run` — таблица кодов не заполняется вообще, job не ставится
    (иначе внешняя транзакция всё равно откатится, но обнуление шло бы по
    отдельному соединению вне неё);
  - `--categories=slug,...` — частичный импорт не должен обнулять остальной
    каталог;
  - выключено в настройках (`Каталог → Настройки импорта →
    «Обнулять остаток товаров, пропавших из выгрузки»`).
- **Предохранитель.** Если доля пропавших товаров превышает
  `zero_out_max_percent` (по умолчанию 20%) от числа товаров в наличии —
  обнуление отменяется целиком, ни один товар не меняется. Признак битого
  или обрезанного CSV от 1С. Оба параметра — `Infrastructure\Settings\CatalogImportSettings`,
  редактируются на той же странице MoonShine.
- Действие идемпотентно: повторный прогон не трогает уже обнулённые товары.
...
├── SeenCodeCollector.php         пишет external_code в catalog_import_seen_codes (обнуление пропавших товаров)
├── Contracts/
...
`ResolveContainerStage::detectCode()` сравнивает нормализованные строки —
регистр приведён к нижнему, точки схлопнуты в пробелы, лишние пробелы убраны.
Значит `'ст. бут.'` матчит и `"ст бут."`, и `"СТ. БУТ."` без дополнительных
строк в маппинге — маркер пиши в каноничном виде, пунктуационные варианты
...
- **`Упаковка = "штучный товар"` — осознанно без контейнера.** Это отсутствие тары, а не её тип: строка не матчит ни один маркер `container_map` (включая `'бут'`), `container_id` остаётся NULL, `ResolveContainerStage` пишет warning `unknown container type`. Не добавляй `'штучный товар' => 'piece'` в маппинг — код `piece` (`ContainerSeeder`) для этого не предназначен, см. `ResolveContainerStageTest::test_piece_package_value_stays_unresolved`.
